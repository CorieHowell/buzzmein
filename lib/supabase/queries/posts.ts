...
export async function getPosts(groupId: string): Promise<Post[]> {
  ...
  const [postsResult, repliesResult] = await Promise.all([
    supabase
      .from("messages")
      .select("id, group_id, user_id, body, image_url, created_at, profiles!messages_user_id_fkey(display_name, avatar_url)")
      .eq("group_id", groupId)
      .is("parent_id", null)
...
export async function getPost(postId: string): Promise<Post | null> {
  ...
  const p = data as unknown as RawPost;
  return {
    id: p.id,
    group_id: p.group_id,
    user_id: p.user_id,
...
export async function getReplies(postId: string): Promise<Reply[]> {
  ...
  const { data } = await supabase
    .from("messages")
    .select("id, parent_id, group_id, user_id, body, created_at, profiles!messages_user_id_fkey(display_name, avatar_url)")
    .eq("parent_id", postId)
